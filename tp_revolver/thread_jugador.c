...
void *thread_jugador(void *arg)
{
    ...
    while (done == 0)
    {
        ...
        if (msg.int_evento == EVT_INICIO)
        {
            ...
            printf("Soy el jugador con id %d y la posicion del tambor es %d\n", args->id, posicion_tambor +1);
            sprintf(buffer, "%d", posicion_tambor + 1);
            enviar_mensaje(id_cola_mensajes, REVOLVER, args->id, EVT_DISPARO, buffer);
        }
        if (msg.int_evento == EVT_FIN)
        {
            printf("Soy el jugador con id %d y me mataron\n", args->id);
            args->vivo = FALSE;
            done = 1;
        ...
        {
            printf("Soy el jugador con id %d y me salvaron\n", args->id);
            args->vivo = TRUE;
            done = 1;
        }
